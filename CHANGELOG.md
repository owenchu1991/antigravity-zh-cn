...
## [2026-06-29] - 修复全英文界面及动态文本汉化失效问题

### 修复 (Fixes)
- **底层汉化机制重构 (核心)**：修复了 Antigravity 最新官方版本更新导致 React 渲染时机变化，从而绕过原有 `createTextNode` 拦截机制，造成整个界面变回英文的问题。现在引入了基于 `MutationObserver` 的全局 DOM 监听兜底机制，不论前端框架如何渲染，都能确保所有文本节点被稳定汉化。
- **后台守护进程稳定性修复**：修复了在极少数情况下（如官方执行文件权限变更或处于打开状态） AppleScript 报 `-48 Duplicate File Name` 错误导致后台驻留失败的问题。
