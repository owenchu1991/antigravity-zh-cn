...
const injectReactHook = () => {
  ...
  (function() {
    ...
    try {
      ...
      // --- MutationObserver Fallback for Dynamic/Shadow Nodes ---
          ...
          if (mut.type === 'childList') {
            mut.addedNodes.forEach(node => {
              if (node.nodeType === Node.TEXT_NODE) {
                ...
                // Recursively translate text nodes inside the element
                const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, null, false);
                let textNode;
                while ((textNode = walker.nextNode())) {
                  const translated = translate(textNode.nodeValue);
                  if (translated !== undefined && translated !== textNode.nodeValue) {
                ...
                if (node.tagName === 'INPUT' || node.tagName === 'TEXTAREA') {
                  if (node.placeholder) {
                    const translated = translate(node.placeholder);
                    if (translated !== undefined) node.placeholder = translated;
                  }
                }
                ...
                inputs.forEach(input => {
                  ...
                  if (translated !== undefined) input.placeholder = translated;
                });
              }
            });
          } else if (mut.type === 'characterData') {
            const translated = translate(mut.target.nodeValue);
            if (translated !== undefined && translated !== mut.target.nodeValue) {
              mut.target.nodeValue = translated;
            }
          }
        });
      });
      ...
      const startObserver = () => {
        if (document.body) {
          ...
          console.log('[ReactHook] DOM MutationObserver Fallback started.');
        } else {
  ...
  const runInjection = () => {
    try {
      require('electron').webFrame.executeJavaScriptInIsolatedWorld(0, [{code: scriptContent}]);
      console.log('[Preload] React hook injected successfully via webFrame.');
    } catch (err) {
      console.error('[Preload] webFrame injection failed:', err);
